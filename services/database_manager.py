...
def get_database_dir():
    ...
    if env_db_dir:
        logger.info(f" 调试：使用环境变量指定的数据库目录: {env_db_dir}")
        return env_db_dir
    ...
    if getattr(sys, 'frozen', False):
        ...
        database_dir = exe_dir / "database"
        logger.info(f" 调试：使用exe目录: {database_dir}")
...
class DatabaseManager:
    ...
    def __init__(self, base_dir: str = None):
        ...
        logger.info(f" 调试：尝试创建数据库目录: {self.base_dir}")
        
        try:
            self.base_dir.mkdir(parents=True, exist_ok=True)
            logger.info(f"✅ 数据库目录创建成功: {self.base_dir}")
        except Exception as e:
            logger.error(f"❌ 数据库目录创建失败: {e}")
            # 如果创建失败，尝试使用用户目录
            import os
            fallback_dir = Path.home() / "KSX_Database"
            logger.info(f" 调试：尝试使用备用目录: {fallback_dir}")
            try:
                fallback_dir.mkdir(parents=True, exist_ok=True)
                self.base_dir = fallback_dir
                logger.info(f"✅ 备用数据库目录创建成功: {self.base_dir}")
            except Exception as e2:
                logger.error(f"❌ 备用数据库目录也创建失败: {e2}")
                raise e2
